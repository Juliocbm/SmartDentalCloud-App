...
Los entity links navegan a vistas de detalle alcanzables desde **múltiples orígenes**. El botón "atrás" debe respetar el origen real del usuario usando `defaultBackRoute`.

> **Guía completa:** Ver `NAVIGATION_BACK_PATTERN.md` para reglas detalladas de `backRoute` vs `defaultBackRoute`, tabla de decisión, y lista de componentes.
