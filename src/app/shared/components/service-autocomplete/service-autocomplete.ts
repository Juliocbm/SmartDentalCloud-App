...
import { DentalServicesService } from '../../../features/invoices/services/dental-services.service';
import { DentalService } from '../../../features/invoices/models/service.models';
...
@Component({
  ...
  templateUrl: './service-autocomplete.html',
  styleUrl: './service-autocomplete.scss'
})
export class ServiceAutocompleteComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['selectedServiceId'] && this.selectedServiceId && this.selectedServiceName) {
      // Preselect from parent inputs
      ...
      this.searchControl.setValue(this.selectedServiceName, { emitEvent: false });
    }
  }
  ...
  private loadServices(): void {
    ...
    this.dentalServicesService.getAll().subscribe({
      next: (services) => {
        ...
        // If preselection was set before load, enrich with full data
        if (this.selectedServiceId) {
          const found = active.find(s => s.id === this.selectedServiceId);
          if (found) {
            this.selectedService.set(found);
            this.searchControl.setValue(found.name, { emitEvent: false });
          }
        }
      },
      error: (err) => {
